...
import phonetize 
from phonetize import is_english_word, phonetization
...
model = load_pretrained_model(model, "/content/Transliteration_deep_learning/Training_1/weights/model_weights.pth")
model.to(device)
model.eval()
...
def new_runner(word, topk=10, checkpoint_path="Transliteration_deep_learning/Training_1/weights/model_weights.pth"): #<-----------------------------
    ...
    in_vec = torch.from_numpy(src_glyph.word_to_vec(word)).to(device)
...
if is_english_word(word):
  eng_ph = phonetization(word);

  tr_eng_ph = new_runner(eng_ph) #transliterate w/ phonetic
  tr_eng_ = new_runner(word); #w/o phonetic

  tr_eng_.insert(1, tr_eng_ph[0])

  print(tr_eng_)

else:
  print(new_runner(word))
